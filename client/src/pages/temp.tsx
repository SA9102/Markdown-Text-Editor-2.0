...
const MainPage = () => {
  ...
  console.log("OUSIDE OF USEEFFECT");
  ...
  */
  ...
  const recGetFilesAndFolders = (data: FileAndFolderTreeType) => {
    for (let i = 0; i < data.length; i++) {
      if (data[i].type === "Folder") {
        ...
        console.log("FOLDER");
        console.log(data[i].parentFolderIds);
  ...
  const handleSaveToDB = async () => {
    // folders = [];
    // files = [];
    // recGetFilesAndFolders(data);
    // console.log(EJSON.stringify(data));
    // console.log(JSON.stringify(data));
    try {
      // const res = await axios({ method: 'post', url: 'http://localhost:3000/saveData', data: { folders: folders, files: files } });
